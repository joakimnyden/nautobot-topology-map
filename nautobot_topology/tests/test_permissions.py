...
class TopologyPermissionsTest(APITestCase):
    def setUp(self):
        ...
        self.site = Location.objects.create(
            name="Test Site Permissions", location_type=self.site_type, status=self.active_status
        )
    ...
    def test_user_with_view_perms_allowed(self):
        ...
        obj_perm = ObjectPermission.objects.create(name="View Topology", actions=["view"])
    ...
    def test_save_layout_requires_change_perm(self):
        ...
        obj_perm = ObjectPermission.objects.create(name="View Only", actions=["view"])
        obj_perm.users.add(self.user)
        obj_perm.object_types.add(ContentType.objects.get_for_model(TopologyLayout))
        obj_perm.save()
