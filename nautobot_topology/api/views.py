...
class TopologyViewSet(ViewSet):
    ...
    def list(self, request):
        ...
        for site in sites:
            ...
            actual_link_count = Cable.objects.filter(
                _termination_a_device_id__in=site_device_ids, _termination_b_device_id__in=site_device_ids
            ).count()
        ...
        response_data = {"status": "success", "data": {"nodes": sites_data, "links": []}}
                    ...
                    links_map.append(
                        {
                            ...
                            "localAs": (
                                str(getattr(getattr(eps[0], "autonomous_system", None), "asn", ""))
                                if len(eps) > 0 and getattr(eps[0], "autonomous_system", None)
                                else None
                            ),
                            "remoteAs": (
                                str(getattr(getattr(eps[1], "autonomous_system", None), "asn", ""))
    ...
    def metrics(self, request, pk=None):
        ...
        cables = Cable.objects.filter(
            _termination_a_device_id__in=device_ids, _termination_b_device_id__in=device_ids
    ...
    def import_cables(self, request):
        ...
        return Response(
            {"status": "success" if not errors else "partial", "created": len(created_cables), "errors": errors}
        )
