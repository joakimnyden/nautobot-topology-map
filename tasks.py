...
def test(context, module=None, coverage=False):
    ...
    if coverage:
        ...
        cmd = (
            "docker compose exec -T nautobot coverage run --source=nautobot_topology "
            f"-m nautobot.core.cli test {test_target} --noinput"
        )
...
def db_export(context, filename="dev_data.json"):
    ...
    context.run(
        f"docker compose exec -T nautobot nautobot-server dumpdata --indent 2 > {filename}", pty=False, in_stream=False
    )
...
def db_import(context, filename="dev_data.json"):
    ...
    context.run(f"docker compose exec -T nautobot nautobot-server loaddata {filename}", pty=False, in_stream=False)
...
def db_export_sql(context, filename="dev_snapshot.sql"):
    ...
    print(f"Exporting database to {filename}...")
    context.run(f"docker compose exec -T db pg_dump -U nautobot nautobot > {filename}", pty=False, in_stream=False)
...
def db_import_sql(context, filename="dev_snapshot.sql"):
    ...
    context.run(f"docker compose exec -T db psql -U nautobot nautobot < {filename}", pty=False, in_stream=False)
...
def setup_dev(context):
    ...
    context.run(
        "docker compose exec -T nautobot bash -c "
        "\"echo 'from django.core.cache import cache; cache.clear()' | nautobot-server shell\"",
        pty=False,
        in_stream=False,
    )
