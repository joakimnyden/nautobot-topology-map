            ...
            if isinstance(lldp_output, list) and lldp_output:
                for entry in lldp_output:
                    ...
                    remote_iface = entry.get(
                        "REMOTE_PORT", entry.get("neighbor_interface", entry.get("NEIGHBOR_INTERFACE", ""))
                    )
                    remote_ip = entry.get(
                        "MANAGEMENT_IP", entry.get("management_ip", entry.get("ADDRESS", entry.get("address", "")))
                    )
...
def normalize_interface_name(name):
    ...
    name = re.sub(r"^xe-?(\d)", r"xe-\1", name)
    name = re.sub(r"^ge-?(\d)", r"ge-\1", name)
    name = re.sub(r"^et-?(\d)", r"et-\1", name)
    name = re.sub(r"^vl(an)?\s?(\d)", r"vlan\2", name)
    name = re.sub(r"^mgmt\s?(\d)", r"mgmt\1", name)
...
def standardize_and_match_neighbors(local_device, raw_neighbors):
    ...
    for neighbor in raw_neighbors:
        ...
        if not possible_devices.exists() and remote_ip:
            ...
            possible_devices = Device.objects.filter(primary_ip4__address__host=remote_ip) | Device.objects.filter(
