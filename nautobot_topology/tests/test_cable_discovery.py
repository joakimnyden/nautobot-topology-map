...
class DiscoverCablesEndpointTest(TestCase):
    ...
    def setUp(self):
        self.user = User.objects.create_superuser(username="cd_user", email="cd@example.com", password="password")
    ...
    def test_device_not_found(self, mock_dev_get):
        ...
        response = self.client.get("/api/plugins/nautobot_topology/topology/bad-pk/discover_cables/")
...
class DevicesEndpointTest(TestCase):
    ...
    def test_devices_site_not_found(self, mock_loc_get):
        ...
        response = self.client.get("/api/plugins/nautobot_topology/topology/bad-pk/devices/")
        self.assertEqual(response.status_code, 404)
        self.assertEqual(response.data["status"], "error")
    ...
    def test_devices_success(self, mock_loc_get, mock_get_locs, mock_dev_filter):
        ...
        response = self.client.get("/api/plugins/nautobot_topology/topology/site1/devices/")
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.data["status"], "success")
...
class ImportCablesEndpointTest(TestCase):
    ...
    def setUp(self):
        self.user = User.objects.create_superuser(username="ic_user", email="ic@example.com", password="password")
        self.client = APIClient()
        self.client.force_authenticate(user=self.user)
    ...
    def test_import_cables_success(self, mock_cable_cls, mock_status_get):
        ...
        with patch("nautobot_topology.api.views.Interface") as mock_iface_cls:
    ...
    def test_import_cables_already_cabled(self):
        ...
        with (
            patch("nautobot.extras.models.Status.objects.get") as mock_status_get,
...
class DiscoveryModuleTest(TestCase):
    ...
    @patch(
        "nautobot_topology.api.discovery.settings.PLUGINS_CONFIG",
        {"nautobot_topology": {"discovery_simulator_enabled": False}},
    )
